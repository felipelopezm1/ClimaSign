...
def generate_image_in_background(prompt, gesture_label, city_name):
    global tracking_enabled, generation_start_time
    ...
    output_folder = os.path.join("generated_images", city_name)
    os.makedirs(output_folder, exist_ok=True)
    filename = os.path.join(output_folder, f"{gesture_label}_{city_name}.png")
    image_pil.save(filename)
    print(f"✅ Image saved: {filename}")

    # Now restart tracking after the generation finishes
    tracking_enabled = True
    sequence_buffer.clear()
    print("🔄 Ready for next gesture after image generation.")
    ...
    if len(sequence_buffer) >= 30: #changed it to more than 90 as it was lagging a lot
        ...
        with torch.no_grad():
            ...
            if gesture_label in ["rainy", "sunny", "hail", "cloudy", "snowy"]:
                ...
                # Save info before launching thread
                current_city = city
                current_condition = condition
                ...
                print(f"🎨 Starting background image generation for: {prompt}")
                tracking_enabled = False
                generation_start_time = time.time()

                threading.Thread(
                    target=generate_image_in_background,
                    args=(prompt, gesture_label, city['name']),
                    daemon=True
                ).start()
